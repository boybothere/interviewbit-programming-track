...
1. The first loop iterates from `i = 0` to `i = N-1`, i.e., **N iterations**.
   Each iteration performs a constant-time operation (`a = a + rand()`). Here, `rand()` is a constant-time operation.

2. The second loop iterates from `j = 0` to `j = M-1`, i.e., **M iterations**.
...
1. Here, `a`, `b`, `i`, and `j` are scalar variables (i.e., similar to primitive data types which can only hold at most single values, unlike arrays which hold multiple values — that's the fundamental understanding) and no additional data structure is used to solve the problem.
